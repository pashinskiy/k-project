            ...
            <div className={classes.root_container}>
                                            ...
                                            {(brand.child.document === null)
                                                ...
                                                :
                                                    <div className={classes.brand_wrapper}>
                                                        <img src={brand.child.document.data.body[0].primary.image.localFile?.childImageSharp?.gatsbyImageData.images.fallback.src} alt={brand.child.document.data.body[0].primary.image.alt ?? "img"} />
                                                    </div>
                                            }
                                        </div>
                                    ))}
                ...
                <div className={classes.promo}>
                    {data.allPrismicCatalog.edges[0].node.data.categories.filter(atr => atr.category.document.data.name === hover)[0].category.document.data.body.filter(slice => slice.slice_type === 'vertical_img').map((promo, i) => (
                        <img src={promo.primary?.catalog_img?.localFile?.childImageSharp.gatsbyImageData.images.fallback.src} alt={promo.primary?.catalog_img.alt ?? "img"} className={classes.img} key={`images_promo ${i}`} />
                    ))}
                </div>
            </div>
        </div>
